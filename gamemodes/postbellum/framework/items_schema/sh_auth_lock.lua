...
ITEM.name = "Кодируемый замок";
ITEM.plural = "Кодируемых замков";
--ITEM.cost = 40;
		...
		if (entity:GetPos():Distance( player:GetPos() ) <= 192) then
			if !LOCKER.storage[entity:EntIndex()] then
				if (!Clockwork.entity:IsDoorFalse(entity)) then
					...
					lockerStruct.locktype = 'auth'
					lockerStruct.state = 0
					lockerStruct.cardItems = {}
					entity:Fire('Lock')
					...
					Clockwork.player:Notify(player, "Эту дверь нельзя закрыть этим замком!");
